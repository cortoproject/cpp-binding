...
corto_int16 cpp_fluentDecl(corto_type type, char *base, cpp_classWalk_t *data) {
    ...
    g_fileWrite(data->hiddenImpl,
      "%s_t::%s_t() : %s_fluent<%s_t>(*this, &m_value) { m_type = corto_type(%s); }\n",
      corto_path(NULL, g_getCurrent(data->g), type, "::"), id, id, id, typeVarId);

    g_fileWrite(data->header, "%s_t(%s& obj);\n", id, id);
    g_fileWrite(data->hiddenImpl,
      "%s_t::%s_t(%s& obj) : %s_fluent<%s_t>(*this, &m_value) { m_type = corto_type(%s); }\n",
      corto_path(NULL, g_getCurrent(data->g), type, "::"), id, id, id, id, typeVarId);
